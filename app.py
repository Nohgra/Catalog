...
import os, httplib2, requests, string, datetime, random, json
...
def showCatalog():
    ...
    items = session.query(Items).order_by(asc(Items.date)).limit(5)
...
def showCategory(category_name):
    ...
    if 'username' not in login_session or creator.id != login_session['user_id']:
        return render_template('p_items.html',
...
# Display single items from catalog
...
def showItem(category_name, item_name):
    ...
    categories = session.query(Category).order_by(asc(Category.name))
    if 'username' not in login_session or creator.id != login_session['user_id']:
        return render_template('p_itemdetail.html',
            item=item,
...
def editCategory(category_name):
    ...
    category = session.query(Category).filter_by(name=category_name).one()
    # Validate, if current user = creator, otherwise redirect him
...
def deleteCategory(category_name):
    categoryDelete = session.query(Category).filter_by(name=category_name).one()
    # Validate, if current user = creator, otherwise redirect him
    creator = getUserInfo(categoryDelete.user_id)
    user = getUserInfo(login_session['user_id'])
    if creator.id != login_session['user_id']:
...
def deleteItem(category_name, item_name):
    itemDelete = session.query(Items).filter_by(name=item_name).one()
    category = session.query(Category).filter_by(name=category_name).one()
    categories = session.query(Category).all()
    ...
    if request.method == 'POST':
        ...
        session.commit()
        flash('Item Deleted '+itemDelete.name)
        return redirect(url_for('showCategory',
